...
def listar_equipas():
    equipas = db.execute('''
        SELECT e.idSigla, e.nome, COUNT(DISTINCT m.ano) n_participacoes 
            FROM Partidas p
...
@APP.route('/equipas/<string:Sigla>/')
def detalhes_equipa(Sigla):
    detalhes = db.execute('''
       SELECT e.nome AS nome, 
            COUNT(CASE WHEN m.vencedor = e.nome THEN 1 END) AS primeiro_lugar,
            COUNT(CASE WHEN m.segundo_lugar = e.nome THEN 1 END) AS segundo_lugar,
            COUNT(CASE WHEN m.terceiro_lugar = e.nome THEN 1 END) AS terceiro_lugar,
            COUNT(CASE WHEN m.quarto_lugar = e.nome THEN 1 END) AS quarto_lugar
        FROM Equipas e 
        NATURAL JOIN Participacoes
        NATURAL JOIN Mundiais m
        WHERE e.idSigla = ?
        GROUP BY e.nome;
    ''', [Sigla]).fetchone()

    return render_template('detalhes_equipas.html', detalhes=detalhes)

@APP.route('/mundiais/<ano>/')
def mundiais_ano(ano):
    participacoes = db.execute('''
        SELECT t1.n_partidas, t2.n_equipas FROM
            (SELECT COUNT(*) n_partidas, m.ano 
                FROM Partidas p
...
def listar_partidas(ano):
    partidas = db.execute('''
        SELECT p.data, p.estadio, p.cidade, p.sigla_eqcasa, p.golos_eqcasa, p.golos_eqfora, p.sigla_eqfora, p.espectadores 
        FROM Partidas p
        NATURAL JOIN Ocurrencias o
        NATURAL JOIN Mundiais m
        where m.ano = :ano
...
def listar_detalhes(ano):
    detalhes = db.execute('''
        SELECT p.data, p.arbitro, p.assistente1, p.assistente2 
        FROM Partidas p
        NATURAL JOIN ocurrencias o
        NATURAL JOIN mundiais m
        WHERE m.ano = :ano
...
def listar_participantes(ano):
    participantes = db.execute('''
        ...
        FROM Equipas e 
        NATURAL JOIN Participacoes pa 
        NATURAL JOIN Mundiais m 
        JOIN Partidas p ON (p.sigla_eqcasa = e.idSigla OR p.sigla_eqfora = e.idSigla)
        NATURAL JOIN Ocurrencias o
        WHERE m.ano = :ano
        GROUP BY e.idSigla
